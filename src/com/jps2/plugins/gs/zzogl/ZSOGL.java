package com.jps2.plugins.gs.zzogl;
...
import com.jps2.plugins.gs.zzogl.gif.GIFDecoder;
import com.jps2.plugins.gs.zzogl.gif.GIFRegHandlers;
import com.jps2.plugins.gs.zzogl.info.PathInfo;
...
public class ZSOGL extends AbstractGSPlugin {
	...
	private final void gifTransferInternal(final PathInfo path, int pMem, int size) {
		while (size > 0) {
			if (path.tag.nloop == 0) {
				GIFtag(path, pMem);
				pMem += 4;
				...
				// if ((g_GameSettings & GAME_PATH3HACK) && path == gs.path3 &&
				// gs.path3.tag.eop) {
				// nPath3Hack = 1;
				// }
					...
					if (path.mode == 1) {
						...
						for (int i = 0; i < path.tag.nreg; i += 4) {
							if (((path.regs >> i) & 0xf) == 11) {
								System.err.println("Invalid unpack type\n");
								path.tag.nloop = 0;
								return;
							}
						}
				...
				if (path == gs.path1) {
					System.err.println(String.format("Reread %d %x %x\n", path.tag.nloop, pMem, pMem - 0x4000));
				}
			}
			...
			switch (path.mode) {
				case 1: // PACKED
				{
					...
					for (; size > 0; size--, pMem += 4) {
						...
						GIFDecoder.handlerPackedReg(gs, reg, convertLongArrayToIntArray(Memories.memoryGS.read128(pMem)));
				...
				case 2: // REGLIST
				{
					...
					// *(u32*)&gs.regs, path->tag.nreg/4);
					assert (path.tag.nloop > 0);
					size *= 2;
					for (; size > 0; pMem += 2, size--) {
						final int reg = (int) ((path.regs >> path.regn) & 0xf);
						GIFDecoder.handlerReg(gs, reg, convertLongArrayToIntArray(Memories.memoryGS.read128(pMem)));
						path.regn += 4;
						if (path.tag.nreg == path.regn) {
							path.regn = 0;
							if (path.tag.nloop-- <= 1) {
				...
				case 4: // Used in the DirectX version, so we'll use it here
					// too.
				{
				...
				default: // GIF_IMAGE
					System.err.println("*** WARNING **** Unexpected GIFTag flag\n");
					path.tag.nloop = 0;
					break;
			}
		...
		// This is case when not all data was readed from one try: VU1 to much
		// data.
		// So we should redone reading from start
		if (path == gs.path1 && size == 0 && path.tag.nloop > 0) {
			System.err.println(String.format("VU1 too much data, ignore if gfx are fine %d\n", path.tag.nloop));
			// TODO: this code is not working correctly. Anyway, ringing work
			// only in single-threadred mode.
			// _GSgifTransfer(&gs.path1, (u32*)((u8*)pMem-0x4000), (0x4000)/16);
		}
	}

	private final void GIFtag(final PathInfo path, final int address) {
		final int[] data = convertLongArrayToIntArray(Memories.memoryGS.read128(address));
		...
		switch (tagflg) {
			case 0x0:
				// TODO - Dyorgio, I need a C pointer lesson
				// path.regs = *(u64 *)(address+2);
				path.regs = address + 2;
				path.regn = 0;
				if (tagpre != 0) {
					GIFRegHandlers.PRIM.handler(gs, new int[] {
						tagprim
					});
				}
	...
	@Override
	public void readFIFO(final int mem) {
		// TODO Auto-generated method stub

	}

	@Override
	public void readFIFO2(final int mem, final int qwc) {
		// TODO Auto-generated method stub
	...
	public static int[] convertLongArrayToIntArray(final long[] longArray) {
